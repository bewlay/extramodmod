##############################################################################
## File: ErebusContinent.py version 2.71
...
## Version History
##
## 2.72 -   (Modified by Terkhen)
##          Improved Clan of Embers starting plots.
##          Asserts will no longer be triggered in Mods that support this feature.
##
## 2.71 -   Fixed a possible exception in starting plot selection
##
## 2.70 -   Added bonus value scale base on flavor start option
##          Added minor weight for jungle to Clan of Embers
##
## 2.65 -   Re-worked starting plot weights, alternates east/west weight per
##             player to spread them out more evenly
##          Player list now grouped by neighbor value so civs with negative
##             neighbor value ar placed first, followed by those with a possitive
##             value.  The list is still shuffled to prevent the same player
##             from always getting the first pick.
##
## 2.64 -   Added support for Tlacatl & Palatinate civs from Orbis
...
##
## 2.63 -   Another shot at Orbis compatability
##
## 2.62 -   Orbis support? Maybe? Hopefully?  Added 'Crystal Plains' support
##          Modified jungle placement to rely more on rainfall than temperature
##
## 2.61 -   Northen plains renamed to 'great plains' and added random fuzz to
##             prevent obvious transitions.
##
## 2.60 -   Fixed a plethora of typos from 2.59, really, this time I swear :)
##          Added sneak peak to a new climate option 'Northern Plains' that
##             converts desert plots to plains based on temperature.
##
## 2.59 -   Change island cleanup, now based on flavor setting and cohesion
##          Adjusted starting plot values for jungle/forest/hill civs
##          Fixed bug in ancient ruin placement
##          Fixed bug in starting area assignment
##
## 2.58 -   Fixed issue with floodplains missing after desert hills flattened
##          Smart climate adjustment for massive continents also takes sea level
##             into consideration
##          Adjusted flavor start options, minimal flavor now ignores preferences
##             for north and west weights
...
##
## 2.57 -   Tweaked smart climate handling of deserts & tundra
##          Added smart climate adjustment for jungles based on southern land
##          Added smart climate recognition of Y wrap maps, now ignores evaluating
##             northern and southern ratios on Y wrap maps.
##          Added safety check when placing bonuses to ensure they don't replace
##             FfH unique features
##          Redesigned normilization code, added ability to replace bad terrain
##             when 'minimal flavor' starting option enabled
##          Replaced previous multiplayer checks with map option for start plots
##
...
## GLOBAL TUNING VARIABLES: Change these to customize the map results
		...
		# ErebusContinent uses InfoTypes that may not be present so it will use
		# the NoAsserts version when available.
		if ("getInfoTypeForStringNoAsserts" in dir(CyGlobalContext())):
			GetInfoType = CyGlobalContext().getInfoTypeForStringNoAsserts
		...
		self.itNoFeature = FeatureTypes.NO_FEATURE
		...
		self.itNoImprovement = ImprovementTypes.NO_IMPROVEMENT
		...
		for x in range(mc.width):
			...
			if mc.UseAdvancedFeatures:
				...
				if plot.isWater() and not waterArea.isLake() and not waterArea.isNone():
					...
					noReef = True
					if plot.isWater() and sm.averageTempMap[i] > mc.TundraTemp * 1.5 and featureReef != -1:
...
class StartingPlotFinder :
	...
	def SetStartingPlots(self):
		...
		#we want the isolationists placed first, then the early rushers and
		#everybody else last
		isolationList = list()
		rushList = list()
		normalList = list()
		...
		shuffledPlayers = isolationList + rushList + normalList
					...
					for xx in range(x-2,x+2):
						for yy in range(y-2,y+2):
							...
							if bonusInfo != None:
								bonusValue += 2
								rawValue += 1
								# Increase the value for resources that can be used right away
...
# Base FfH2 Civs
	...
	pref.westValue = 0
	pref.edgeValue = -1
	if plotRules == 1: # Don't burry them in jungles
		pref.marshValue = -2
		pref.jungleValue = -1
	if plotRules == 2: # Don't place them in jungles
		pref.marshValue = -2
		pref.jungleValue = -2
	civPreferenceList.append(pref)

# modmod Civs
	pref = CivPreference(GetInfoType("CIVILIZATION_AUSTERIN"))
	pref.desertValue = 0
	pref.plainsValue = 1
	pref.grassValue = 1
	pref.marshValue = 0
