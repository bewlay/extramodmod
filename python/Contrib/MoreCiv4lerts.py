...
#RevolutionDCM start - extra civlerts
	def getCheckForWarTrades(self):
		return self.options.isShowWarTradeAlert()
#RevolutionDCM end
...
#RevolutionDCM start - extra civlerts
		if (BeginTurn and self.getCheckForWarTrades()):
			currentTrades = self.getWarTrades(activePlayer, activeTeam)
			newTrades = currentTrades.difference(self.PrevAvailWarTrades)
			self.PrevAvailWarTrades = currentTrades
			if (newTrades):
				players = self.buildPlayerString(newTrades)
				message = localText.getText("TXT_KEY_MORECIV4LERTS_TRADE_WAR", (players,))
				self._addMessageNoIcon(iActivePlayer, message)
#RevolutionDCM end		
