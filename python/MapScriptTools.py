...
#  - add Marsh terrain on maps for mods which use them
#  - add Deep Ocean terrain on maps for mods which use them
#  - add new features on maps for mods which use them
...
#  - organize starting-positions for teams
...
#    Please tell me.
#
#  - MAC compatibility is unknown as the MAC uses Python 2.3.
#    - Sets are not built-in in Python 2.3.
#      I believe I used them only in adjustShelfTrenches() & rimRivers() though.
#      ! I think this is fixed. - Thanks EmperorFool
#    - Also new in Python 2.4 was the use of the keyword parameter 'key' within the sort() method of lists.
#      I used key in makeMapLegend() & showContinents().
#      ! I think this is fixed. - Thanks Xyth & EmperorFool
#
#    I've tried to fix all those MAC issues, both here and with the associated maps.
#    Maybe now it's compatible? Please tell me.
...
#            - as defined by the prerequisites for the Towers of Alteration/Divination/Necromancy/Elements;
#              all other mana types are considered mana for meta-magics
#            - as defined by the mana type tech, that is the prerequisite of
#              building the appropiate node on the mana boni
#         Changed, in buildTerrainMap(), buildFeatureMap(),buildBonusMap()
#            the showPlots parameter now defaults to False
#         Improved, recognize 'Master of Mana', 'History Rewritten' mods
#         Improved, put addGoodies() into the template
#         Improved, whenever a reef is build in the ocean,
#            there is now a chance that it expands into a somewhat longish chain of reefs
...
#         Fixed, incompatibility with 'History in the Making', 'Quod Capita',
#           and possibly some other mods using newer BUG versions
...
#         Changed [Planetfall], Tundra conversion in planetFallMap.mapPfallTerrain() now:
#           Tundra -> 33% FlatPolar, 16% RockyPolar, 33% FlatMoist, 16% RockyMoist
#         Changed [Planetfall], Highland generation in planetFallMap.buildPfallHighlands()
#           now gives more hills/peaks, which are more clustered together - like Planetfall
#         Changed [Mars Now!], converting ocean to desert: some few hills are created and
#           at higher latitudes tundra or even some ice may be created
...
#         Improved, less chance for 'BigBog' and 'BigDent' to start on small island and
#            better chance to start inside bigger continent
...
#         Added, parameter to mapRegions.buildBigBog() and mapRegions.buildBigDent() to control
#            chances for each special region to exist; slightly lowered default chance (now 66%).
...
#         Added [Planetfall], parameter and result to planetFallMap.buildPfallHighlands()
#           to enable use before plots are generated
#         Added [Planetfall], name signs for already known Planetfall regions:
#           'Mount Planet', 'Garland Crater', 'Manifold Nexus'
#           'The Great Dunes', 'Pholus Ridge', 'The Ruins', 'Borehole Cluster'
#         Added [Mars Now!], new global variable: bSandsOfMars, denoting a map on Mars, where
#           all surface water is converted to desert.
...
#	print "[MST] " + sVersion
	...
	if bFF:
		n1 = sVersion.find( '(' )
		n2 = sVersion.find( ')' )
		modName = " Rise from Erebus " + sVersion[n1+1:n2]
	## FFH - ExtraModMod
	sVersion = unicode(CyTranslator().getText("TXT_KEY_VERSION", ()))		# ExtraModMod versions follow the ExtraModMod X.Y.Z format.
	bEMM = (sVersion.find("ExtraModMod") > -1)
	if bEMM:
		modName = " " + sVersion
	...
	bStorm           = ( efStorm >= 0 )
	efKelp           = git('FEATURE_KELP')				# terain_coast
	bKelp            = ( efKelp >= 0 )
...
# buildBigBogs( iBogs=None, chBog=66 )
...
class MapRegions:
	...
	def buildBigBogs( self, iBogs=3, chBog=66 ):
		...
		for i in range( min(iBogs, iif(bMars, 2, 3)) ):
			if choose( iif(bMars, chBog/2, chBog), True, False ):
				...
				if len( validPlots ) > 0:
					...
					if not bMars:
						if plot.isWater():
							mapSetSign( namePlot, bogList[i][2]+" Lake", self.noSigns )
						else:
							mapSetSign( namePlot, bogList[i][2]+" Moor", self.noSigns )
...
#		print "[MST] ======== MapRegions:theBigBog()"
	...
	def buildBigDents( self, iDents=3, chDent=66 ):
		...
		if not (bMars or bPfall):
			dentNames.append( {	'Volcano': "Witches Cauldron",
										'Peak':    "Warlocks Hat",
										'Flat':    "Fairies Glen",
										'Water':   "Satyrs Lake",
										'Hills':   "Wizard Mountains" } )
		elif bPfall:
			dentNames.append( {	'Volcano': "Smocker",
										'Peak':    "Highest Ridges",
										'Flat':    "Murphy's Folly",
...
#							sprint += "[MST] @ %3i,%2i: Hills -> Peak \n" % ( plot.getX(), plot.getY() )
	...
	def buildElementalQuarter( self, chEQ=33 ):
...
#				printList(tempList, "template after %i:" % (i*90), rows=1, prefix="[MST] ")
	...
	def addBigBogExtras( self ):
		if bMars: return		# no BigBog names on Mars
...
class FeaturePlacer:
	...
	def placeHauntedLands( self, chHaunted=6 ):
		...
		if bRifE: chHaunted = chHaunted / 3					# 'Rise from Erebus' already places Haunted Lands
		...
		for iTower in range( len(towers) ):
			for iBuilding in range( gc.getNumBuildingInfos() ):
				if gc.getBuildingInfo(iBuilding).getType() == towers[iTower]:
					towerMana[iTower] = []
					i = 0
					while gc.getBuildingInfo(iBuilding).getPrereqOrBonuses(i) in range( gc.getNumBonusInfos() ):
...
'''
MapScriptTools Manual and API  Ver.: 1.01
=========================================
...
Earth3_163_mst.py             by jkp1187 --> http://forums.civfanatics.com/showthread.php?t=253927
   ...
   - Added Himalayas, Andes, Rockies (each a single BigDent at special place)
   - Signs for Special Regions are not shown
...
Medium_and_Small_111_mst.py   by Sirian (Civ4 original)
...
PerfectWorld_206a_mst.py      by Cephalo --> http://forums.civfanatics.com/showthread.php?t=310891
   - I had to fiddle a bit in generateTerrainTypes() to transform the generated terrainList
   - Uses default river-system. Wasn't able to insert the new rivers of mapRegions.
...
Tectonics_316a_mst.py         by Laurent Di Cesare --> http://forums.civfanatics.com/showthread.php?t=149278
   - Adjusted starting-plot creation using: 'Temudjins Cool Starting Plots'
   - Two different methods to create Planetfall landscape.

Planetfall_101_mst.py        by Maniac --> http://forums.civfanatics.com/showthread.php?t=252829
   - Convert the other way around: from Planetfall to Vanilla BtS, FFH and Mars
