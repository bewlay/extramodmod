...
public:
  ...
  inline void ClearSwapCircle(void){
	swap_loop.clear(); }
  ...
  // search a swap circle using BFS
  bool SearchSwapCircle(cell& _start);
  ...
  // given a swap circle, tasks are swapped, matrix & assign-vec are updated
  void SwapTasks(void);
...
private:
  ...
  set<uint> exploited_rows;	// rows traversed during searching circle
  set<uint> exploited_cols;	// cols traversed during searching circle
